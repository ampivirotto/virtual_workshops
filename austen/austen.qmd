...
One thing you might want to understand about your text is how often each word appears. We can do this by counting the occurrences of each word. Try changing the `sample_text` variable to see how that changes the result from the cell. Try adding more words or taking away some words to see how that affects the words counted. 
...
## HERES WHERE YOU SHOULD UPDATE THE CODE
sample_text = "Pride and Prejudice is a novel by Jane Austen. It is full of wit and social commentary."

## this regex finds all words and makes them lowercase
words = re.findall(r'\b\w+\b', sample_text.lower()) 

## this prints all the words to the screen 
print("Words:", words)

## this prints the total number of words 
...
# Create a Pandas DataFrame for the table
df_filtered = pd.DataFrame(list(filtered_word_counts.items()), columns=['Filtered Word', 'Frequency'])
...
In the actual Colab notebook, we would calculate the sentiment scores using a library like [NLTK](https://www.nltk.org/). For this sample code though, we will use dummy data sentiment scores. How does changing the scores in the list change your result?
